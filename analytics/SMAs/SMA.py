...
def analyze_manual_ranges(streaks_with_indices):
    ...
    summary = [conteo_objetivo, conteo_limite, conteo_sin_resultado]
    ...
    return all_parts, serie, summary

# Ejecución del análisis (asumiendo que df ya está cargado)
df = calculate_above_sma(df)
streaks_info = find_streaks(df)
all_parts, serie, summary = analyze_manual_ranges(streaks_info)

def get_sma_data():
    return {
        "all_parts": all_parts,
        "estadisticas": {
            "promedio": float(round(serie.mean(), 2)),
            "mediana": float(round(serie.median(), 2)),
            "std": float(round(serie.std(), 2)),
            "iqr": float(round(serie.quantile(0.75) - serie.quantile(0.25), 2)),
            "min": float(serie.min()),
            "max": float(serie.max()),
            "cv": float(round(serie.std() / serie.mean(), 2))
        },
        "resumen": {
            "objetivo_delta": OBJETIVO_DELTA,
            "limite_delta": LIMITE_DELTA,
            "max_candles": MAX_CANDLES,
            "conteo_objetivo": summary[0],
            "conteo_limite": summary[1],
            "conteo_sin_resultado": summary[2]
        }
